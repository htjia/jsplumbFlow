<template>
  <div v-if="showDiaLogs" class="viewabs">
    ...
    <div v-if="!isPhone" :style="{ background: viewList[isActives].background}" class="viewss">
      <grid-layout
        v-if="!isview"
    ...
    <div v-if="isPhone" :style="{ background: viewList[isActives].background,width: screeWidth + 'px'}" class="viewssx">
      <grid-layout
        v-if="!isview"
        :layout.sync="viewList[isActives].layout"
        :col-num="24"
      ...
      >
        ...
        >
          ...
          <div v-if="!item.isError" :style="{ height: ((32 * item.h) + 20) + 'px',width: screenWidth + 'px' }" style="padding-top:5px" @click="getinfo(item, index)">
            <chart v-loading="listLoading" v-if="item.type === 'bar' || item.type === 'zuhe' || item.type === 'loudou' || item.type === 'duidie' || item.type === 'line' || item.type === 'pie' || item.type === 'tiao' || item.type === 'scatter'" :id="item.type + item.i + 'x'" :options="item.options" :style="{ height: ((32 * item.h) + 30) + 'px', width: screenWidth + 'px' }" style="padding: 10px" @chartAllClick="chartAllClick"/>
            <el-table
              v-loading="listLoading"
              v-if="item.type === 'table'"
              :data="item.options.list"
            ...
            <div v-loading="listLoading" v-if="item.type === 'tstab'" :id="item.type + item.i + 'x'" :style="{ height: 32 * item.h + 'px', width: screenWidth + 'px' }" class="rulte" style="padding-top:20px;">
              &nbsp;
            </div>
...
<script src="./publicView.js"></script>

<style scoped>
@import url(../presentation/index.css);
</style>
