  ...
  methods: {
    flowInputFun() {
      ...
      this.data.lineList.map(item => {
        if (item.to === this.node.id) {
          formList.push(item.from)
        }
      })
      this.data.nodeList.filter((node) => {
        if (node.id === this.node.id) {
          node.config['left'] = formList[0]
          node.config['right'] = formList[1]
        }
      })
      const obj = {
        dag: this.data
      ...
      flowInput(params).then(res => {
        this.tableOneList = res[0].fields
        this.tableTwoList = res[1].fields
